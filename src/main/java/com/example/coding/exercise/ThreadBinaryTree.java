 ...
 * @Title: 二叉树相关操作
 * @Description：一个实际的应用：
 *                  当路由器使用CIDR，选择下一跳的时候，或者转发分组的时候，
 *                  通常会用最长前缀匹配（最佳匹配）来得到路由表的一行数据，
 *                  为了更加有效的查找最长前缀匹配，使用了一种层次的数据结构中，
 *                  通常使用的数据结构为二叉线索。
 * @Author： Uncle Ming
...
public class ThreadBinaryTree {
    ...
    public static void main(String[] args) {
        ...
        Node root3 = createBinaryTree(array, 0);
        ThreadBinaryTree tree3 = new ThreadBinaryTree();
        tree3.postThreadOrder(root3);
        System.out.println("\n后序按后继节点遍历线索二叉树结果：");
        tree3.postThreadList(root3);
    }
}
